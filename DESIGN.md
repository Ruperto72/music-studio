...
| `icons.js` | Draws the app icon and writes every file the manifest, `index.html` and `sw.js` declare — `icons/icon.svg`/`icon-maskable.svg` written directly, the six PNGs screenshotted off them via `cdp.js`. Checked in for the same reason `shots.js` is: the previous icons came from a one-off script nobody kept, so changing the icon meant writing a PNG encoder from scratch. Now it's `node icons.js`. No exports — it's a command, not a module. |
