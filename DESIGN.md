...
| `cdp.js` | Shared Chrome DevTools Protocol plumbing — `findBrowser()`, `launchChrome()`, `openPage()`, the `CDP` class — behind `verify.js`, `shots.js` and `icons.js`. Exists because the same ~120 lines were hand-copied into two tools and drifted (`shots.js` shipped without the hardcoded browser install-path list its sibling had); a third copy was about to be written when this was pulled out instead. Also `requireFreePort()`, which both server-spawning tools call before starting their own `dev-server.js`: each spawns a server and then waits for the port to answer, and `waitForHttp()` cannot tell whose answer it got — a spawn that dies of `EADDRINUSE` leaves the wait succeeding against whatever was already listening. A run whose server outlived it (a timeout, a Ctrl-C) therefore made every later run drive that abandoned tree while reporting confident verdicts about code the browser never loaded. `verify.js` additionally kills its server from a `SIGINT`/`SIGTERM` handler, since the `finally` that normally cleans up only runs when the process gets to finish. |
...
| `verify.js` | A permanent, dependency-free headless-browser smoke test — drives the app over the Chrome DevTools Protocol (`WebSocket` + JSON-RPC, Node built-ins only) through a handful of core interactions and fails on any wrong expectation or console error/exception. Not a full test suite; a reusable regression check. Its first two steps never open a browser: `auditBundledSongs()` reads `songs/*.json` and checks every field against what `applySavedMix()` accepts, and `auditIcons()` checks the manifest, `index.html`'s `<link>`s and `sw.js`'s `SHELL_URLS` both against each other and against `icons/` on disk — each with its constants extracted from the real files so the audit cannot drift from what the app actually does. They run first because a bad example ships to everyone who opens the Songs menu, and because hearing about either costs seconds rather than the browser run's minutes. Every step resets the app itself, which is what makes `--only <substring>` honest: a step run alone means what it means in the suite, and one step is a minute instead of the whole suite's twenty. All 67 steps have been run against a copy of the app with the thing they describe deliberately broken — see `DONE.md` for what that turned up, including the two rules it cost the most to learn (check that an injection actually reaches the code the step exercises before believing a verdict, and run one injection at a time). |
